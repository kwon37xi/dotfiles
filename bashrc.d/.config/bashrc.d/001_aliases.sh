...
# apt update/upgrade
alias sysup='pushd . && sudo apt update && sudo apt -y upgrade && sudo apt  -y autoremove && sudo apt clean && flatpak update -y && sudo snap refresh && brew update && brew upgrade && brew autoremove && brew cleanup --prune=all && sdk update && sdk upgrade && cd ~/.dotfiles && git pull --rebase && cd ~/.dotfiles-private && git pull --rebase && popd'
